...
const LandingPage = () => {
  ...
  React.useEffect(() => {
    if (currentUser) {
      if (userRole === 'guest') { // Assuming 'guest' role exists
        navigate('/guest');
      } else if (['admin', 'manager', 'staff', 'room_service'].includes(userRole)) { // Added room_service
        // Redirect staff/admin/room_service to a relevant dashboard
        // Defaulting to admin dashboard for now, can be refined
        navigate('/admin/dashboard');
      }
      // Add handling for other roles or default redirect if needed
    }
  }, [currentUser, userRole, navigate]);
  // console.log("LandingPage: currentUser:", currentUser, "userRole:", userRole); // Keep for debugging if needed

  const handleAdminLogin = () => {
    // Navigate to a login page, potentially passing a target role or redirect path
    navigate('/login', { state: { role: 'admin' } }); // Assuming login handles role state
  };

  const handleGuestLogin = () => {
    // Navigate to a login page, potentially passing a target role or redirect path
    navigate('/login', { state: { role: 'guest' } }); // Assuming login handles role state
  };
  ...
  // Render login selector if not logged in
  return (
